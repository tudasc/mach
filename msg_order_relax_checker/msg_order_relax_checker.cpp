...
bool check_call_for_conflict(CallBase *mpi_call) {
	...
	std::set<BasicBlock*> already_checked; // be aware, that revisiting the current block micht be necessary if in a loop
	...
	while (next_inst != nullptr) {
		...
		if (auto *call = dyn_cast<CallBase>(current_inst)) {
			if (is_mpi_call(call)) {
				// check if this call is conflicting or if search can be stopped as this is a sync point
		...
		if (current_inst != nullptr) // if not discovered to stop analyzing this code path
				{
			if (current_inst->isTerminator()) {
				for (unsigned int i = 0; i < current_inst->getNumSuccessors();
					...
					if (already_checked.find(next_block)
							== already_checked.end()) {
		...
		if (next_inst == nullptr) {
			if (!to_check.empty()) {
				...
				already_checked.insert(bb); // will be checked now, so no revisiting necessary
...
static RegisterStandardPasses RegisterMyPass(
		PassManagerBuilder::EP_ModuleOptimizerEarly, registerExperimentPass);
