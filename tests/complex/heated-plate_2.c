...
#define NUM_ITER 16384
  ...
  for (iterations = 0; iterations < NUM_ITER; ++iterations) {
    ...
    if (pre != -1) {
      MPI_Isend(w[i], N, MPI_DOUBLE, pre, tagPre, MPI_COMM_WORLD, &reqSendPre);
    ...
    if (nxt != -1) {
      MPI_Isend(w[i], N, MPI_DOUBLE, nxt, tagNxt, MPI_COMM_WORLD, &reqSendNxt);
  ...
  if (rank == 0) {
    MPI_Reduce(MPI_IN_PLACE, &diff, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
