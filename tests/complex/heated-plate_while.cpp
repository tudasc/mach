...
// nur auf rank0
	...
	while (epsilon <= diff) {
		...
		if (pre != -1) {
			MPI_Isend(w[i], N, MPI_DOUBLE, pre, tag, MPI_COMM_WORLD,
					&reqSendPre);
			//Empfangene Nachicht= 0te Zeile der Neuen Matrix
			MPI_Irecv(w[i - 1], N, MPI_DOUBLE, pre, tag, MPI_COMM_WORLD,
					&reqRcvPre);
		}
		//Berechne Zeile N-1
		i = size - 1;
		...
		if (nxt != -1) {
			MPI_Isend(w[i], N, MPI_DOUBLE, nxt, tag, MPI_COMM_WORLD,
					&reqSendNxt);

			MPI_Irecv(w[i + 1], N, MPI_DOUBLE, nxt, tag, MPI_COMM_WORLD,
					&reqRcvNxt);
		}
