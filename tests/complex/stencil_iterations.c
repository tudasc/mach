...
// comparing to stencil_allreduce: different msg tags, but the order of messages
// must conform to the order of iterations
...
int main() {
  ...
  while (done < 100) {
    ...
    if (rank % 2 == 0) {
      ...
      MPI_Recv(&data[0], 1, MPI_INT, pre, MSG_TAG_A, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Send(&data[1], 1, MPI_INT, next, MSG_TAG_A, MPI_COMM_WORLD);

      MPI_Recv(&data[N - 1], 1, MPI_INT, next, MSG_TAG_B, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Send(&data[N - 21], 1, MPI_INT, pre, MSG_TAG_B, MPI_COMM_WORLD);
    }
