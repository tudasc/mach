...
int main() {
  ...
  while (done < 1073741824) {
    ...
    // a halo exchange
    if (rank % 2 == 0) {
      MPI_Send(&data[0], 1, MPI_INT, next, MSG_TAG, MPI_COMM_WORLD);
      MPI_Recv(&data[1], 1, MPI_INT, pre, MSG_TAG, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);

      MPI_Send(&data[N - 2], 1, MPI_INT, pre, MSG_TAG, MPI_COMM_WORLD);
      MPI_Recv(&data[N - 1], 1, MPI_INT, next, MSG_TAG, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    } else {

      MPI_Recv(&data[0], 1, MPI_INT, pre, MSG_TAG, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Send(&data[1], 1, MPI_INT, next, MSG_TAG, MPI_COMM_WORLD);

      MPI_Recv(&data[N - 1], 1, MPI_INT, next, MSG_TAG, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Send(&data[N - 21], 1, MPI_INT, pre, MSG_TAG, MPI_COMM_WORLD);
    }
